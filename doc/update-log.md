...
1. **暂存工单支持生成报价单**
   - 施工中工单底部新增「生成报价单」按钮（绿色主题）
   - 调用分享画布生成报价单图片，标题改为"XX门店 · 报价单"
   - 报价单隐藏"施工中"状态标签和支付方式，适合发给客户确认

2. **暂存工单支持作废**
   - 云函数 `repair_main.voidOrder` 放开状态限制，施工中工单也可作废
   - 暂存工单无库存扣减，作废时库存回滚自然跳过

### OCR 与页面优化

3. **OCR 车牌识别迁移至云函数 action**
   - `cameraScan` 车牌识别改为调用 `repair_aux` 云函数 action，不再依赖客户端直调
   - 识别更稳定可靠

4. **Dashboard 新增"快速入门"引导**
   - 首页顶部新增快速入门入口，新用户可快速了解系统功能
   - 链接到 `doc/快速入门.md` 文档

5. **Pro 页面改造为"使用帮助"**
   - `proUnlock` 页面全新改版，功能说明更清晰
   - 整合 Pro 功能介绍、使用指南和权益说明

### 部署注意

⚠️ **需部署 `repair_main` 和 `repair_aux` 云函数**
...
6. **四层测试体系搭建**
   - 测试目录 `tests/`：单元测试 + 集成测试 + 端到端测试 + 手动测试清单
   - `jest.config.js` 配置文件
   - `doc/真机测试清单.md` 真机测试覆盖

7. **Common 模块同步机制**
   - `cloudfunctions/shared/` 公共模块（auth.js、db-utils.js）
   - `scripts/sync-common.js` 同步脚本：`npm run sync:common` 同步到各云函数
   - `npm run check:sync` 验证一致性

### 前端

8. **更新日志页面**
   - 🆕 `pages/changelog/`（4文件）— 系统更新日志页，向用户展示版本演进
   - 按时间倒序排列，仅展示对用户有价值的功能演进
   - 新增问题反馈入口，可通过小程序客服或拨打电话反馈

9. **开单服务项数据结构优化**
   - `_serviceItemsArr` 字段迁移收尾：`repair_main` createOrder/editOrder/useBenefit 统一写入新格式
   - `orderDetail` / `orderAdd` 读取优先新格式，旧格式兜底兼容
   - `repair_aux` AI月报查询不再依赖旧字段
...
12. **OCR callRepair 迁移**
    - 车牌识别从客户端直调改为云函数 `repair_aux` action 调用
    - `ocrHelper.js` 重构，统一错误处理

13. **_serviceItemsArr 字段迁移**
    - `repair_main` 新建/编辑工单统一写入 `_serviceItemsArr`
    - `orderDetail` 详情页优先读取新格式，旧 `serviceItems/serviceAmounts` 兜底
    - `repair_aux` AI月报查询投影同步更新

14. **月报兜底检查增强**
    - `repair_aux` 新增边界检查，防止无数据场景报错
...
1. **商品模板管理中心（全新）**
   - 新增 `pages/product/productTemplateAdmin/` 模板管理页面
   - 支持模板的增/删/改/查，分类管理（机油/轮胎/刹车系统等10分类）
   - proUnlock 页面新增模板管理入口按钮

2. **模板导入功能增强**
   - `productTemplateImport.js` 重构：新增批量导入预览、导入进度展示
   - 搜索防抖优化至 300ms，提升输入响应
   - 已导入模板显示「已导入」状态标记，支持一键切换上下架

3. **Dashboard 进销存入口集成**
   - 首页「进销存管理」功能入口网格
   - 员工角色下隐藏进销存入口（`wx:if="{{!isStaff}}"`）
...
5. **入库记录查询优化**
   - `productStockIn` 搜索增强，支持按商品名和供货商双维度搜索

### 数据库变更

6. **新增云数据库集合**
   - 需在云开发控制台确认 `repair_product_templates` 集合已创建（v6.3.0 已建）

### 部署注意

⚠️ **无新增云函数 action**（复用已有的 `repair_inventory` 云函数，无需重新部署）

**变更统计：** 15 文件（+1357 / -280 行）
- 🆕 `pages/product/productTemplateAdmin/`（4文件，177行JS + 70行WXML + 273行WXSS）
- 📝 `pages/proUnlock/`（3文件，113行JS + 448行WXML重排 + 135行WXSS）
- 📝 `pages/dashboard/`（2文件，JS + WXML 各增入口网格）
- 📝 `pages/product/productTemplateImport/`（3文件，搜索+批量导入增强）
- 📝 `pages/product/productStockIn/`（2文件，入库搜索增强）
...
1. **cameraScan 相机清晰度修复**
   - 拍照质量从 `'low'` 提升至 `'normal'`，消除 OCR 识别因图片过糊导致的识别失败
   - 减少不必要的相机预览帧（`frame-limited`），降低性能消耗

### 功能新增

2. **proUnlock 联系客服区新增电脑版入口**
   - 下载APP入口后新增「💻 电脑版」入口行（橙色渐变样式）
   - 点击弹出提示「联系客服获取电脑版访问权限」
   - 引导用户联系客服开通 PC 管理后台权限

**变更统计：** 3 文件（+31 / -0 行）
- `pages/proUnlock/proUnlock.js` / `.wxml` / `.wxss`

**向后兼容性：** ✅ 纯前端增量改动，不影响现有流程

---

## v6.3.2（2026-05-18）

### 功能新增：蒙层相机页 + 车牌/VIN 双模式 OCR 识别

1. **全新 cameraScan 蒙层相机页**
   - 独立相机扫描页面，支持 车牌（plate）和 车架号（VIN）双模式
   - 自定义蒙层扫描框：车牌模式蓝色框 + VIN 模式红色框，视觉区分
   - 支持拍照/相册选择双入口采集图片
   - 识别结果通过 `wx.navigateTo` + `events` 回调回传
   - quality 采用 `'normal'` 兼顾清晰度与性能

2. **10 个页面集成车牌拍照识别**
   - 新增车辆（carAdd）、编辑车辆（carDetail）、快速开单（orderAdd）、工单列表（orderList）
   - 车辆列表（carList）、会员列表（memberList）、新增会员（memberAdd）
   - 查车单（checkSheet）、查车单列表（checkSheetList）
   - Dashboard 首页快速搜索区

3. **VIN 车架号 OCR 识别**
   - 新增车辆页、编辑车辆弹窗的车架号输入框旁新增拍照按钮
   - 识别结果确认后自动回填

### 交互优化

4. **拍照按钮视觉统一**
   - 所有页面拍照按钮统一为 📷 相机图标，蓝底圆形风格
   - 按钮位置统一在输入框右侧，保持一致性

**变更统计：** 15 文件（+1139 / -109 行）
- 🆕 `pages/cameraScan/`（4文件，JS 217行 + WXML 99行 + WXSS 359行）
- 📝 10 个业务页面 OCR 按钮集成
- 📝 `app.json`（注册 cameraScan 路由）
